...
from text_cnn_pretrain import TextCNN
...
def get_embedding_from_model(baidu_vocab_index):
    f_baidu_embd = open(
        os.path.join(FLAGS.pre_train_data,
                     'model_%d.emb' % FLAGS.embedding_dim), "rb")
    bytes = 4 * FLAGS.embedding_dim
    baidu_embedding_matrix = np.array([])
    for i in xrange(len(baidu_vocab_index)):
...
def vocabulary_fit_baidu(x_text):
    baidu_index = dict()
    with open(os.path.join(FLAGS.pre_train_data, 'baidu.dict'), "r") as f:
        for line_index, line in enumerate(f):
            word = line.strip().split()[0]
            baidu_index[word.decode('utf-8')] = line_index
    user_dict = []
    baidu_dict = ["<unk>"]
    known_vocabulary = {"<unk>": {'num': 0, 'id': 0}}
    token_id = 1
    for tokens in tokenizer(x_text):
        for token in tokens:
            if token not in known_vocabulary:
                if token in baidu_index:
                    baidu_dict.append(token)
                else:
                    user_dict.append(token)
                known_vocabulary[token] = {}
                known_vocabulary[token]['num'] = 1
    ...
    baidu_embedding_matrix = get_embedding_from_model(baidu_vocab_index)

    vocabulary = user_dict + baidu_dict

    categorical_vocabulary = learn.preprocessing.CategoricalVocabulary(
        unknown_token="<unk>")
    [
        categorical_vocabulary.add(token, known_vocabulary[token]['num'])
        for token in vocabulary
    ]

    vocab_sizes = (len(user_dict), len(baidu_dict))
    return categorical_vocabulary, vocab_sizes, baidu_embedding_matrix

    # In[ ]:


def adapt_data_with_baidu():
    ...
    vocabulary, vocab_sizes, baidu_embedding_matrix = vocabulary_fit_baidu(
        x_text)
    ...
    print('user_defined vocabulary size: %d, pretrain vocabulary size: %d' %
          (vocab_sizes[0], vocab_sizes[1]))

    return x, y, vocab_sizes, baidu_embedding_matrix


def build_dataset(x, y):
    """Get train/validate/test data from file.
...
def training():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            sess.run(cnn.W_pretrain.assign(baidu_embedding_matrix))
